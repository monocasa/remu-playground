...
void Dissector::dissect(uint32_t instr, uint64_t addr)
{
	...
	case 0: case 1: case 2: case 3: {
		...
		else if( (instr & 0x0FF00000) == 0x02000000 ) {
			onAndImm(getCc(instr), false, getReg3(instr), getReg4(instr), getImm12(instr));
		}
		else if( (instr & 0x0FF00000) == 0x02100000 ) {
			onAndImm(getCc(instr), true, getReg3(instr), getReg4(instr), getImm12(instr));
		}
